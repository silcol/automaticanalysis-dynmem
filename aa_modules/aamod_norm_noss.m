...
switch task
    ...
    case 'doit'
        ...
        streamsI=aap.tasklist.currenttask.inputstreams.stream{:};
        
        Simg = aas_getfiles_bystream(aap,subj,streamsI);
        ...
        if ~all(cellfun('isempty', strfind(aap.tasklist.currenttask.outputstreams.stream, 'normalisation')))
            ...
            % segmentation.
            streamsO=aap.tasklist.currenttask.outputstreams.stream;
            if any(strcmp(streamsO, streamsI))
                if (aap.tasklist.currenttask.settings.usesegmentnotnormalise)
                    aap=aas_desc_outputs(aap,subj,streamsI, strvcat( ...
                        fullfile(mSpth,['m' mSfn mSext]), ...
                        fullfile(Spth,['w' Sfn Sext])));
                else
                    aap=aas_desc_outputs(aap,subj,streamsI, fullfile(Spth,['w' Sfn Sext]));
                end
            else
                aas_log(aap, false, sprintf('Could not save output %s, %s stream unchanged by this module', streamsI, streamsI))
            end
        end
