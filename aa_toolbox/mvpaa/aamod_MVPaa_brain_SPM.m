...
switch task
    case 'doit'
        ...
        if aap.tasklist.currenttask.settings.ggmfit == 1
            fprintf('GGMfit-ting T-values and contrasts... \n')
            for s = 1:2
                for c = 1:length(MVPaa_settings.contrasts)
                    % Only non-zero non-nan values of image...
                    M = and(isfinite(Statistics(:,c,s)), Statistics(:,c,s)~=0);
                    
                    if sum(M) == 0
                        error('No values in our mask...')
                    end
                    
                    % First we try the ggm model (Gaussian/Gamma)
                    ggmmix = ggmfit(Statistics(M,c,s)', 3, 'ggm');
                    ...
                    if ~isfinite(ggmmix.mus(1)) || ggmmix.mus(1) == 0 ...
                        ...
                        ggmmix = ggmfit(Statistics(M,c,s)', 2, 'ggm');
                        
                        if any(isnan(ggmmix.mus)) || any(isnan(ggmmix.sig))
                            aas_log(aap,1,'Error in ggm, mu and/or sigma are NaN')
                        end
                    end
                    
                    if ggmmix.sig(1) > eps
                        P = (Statistics(M,c,s) - ggmmix.mus(1)) ./ ggmmix.sig(1);
                    elseif ggmmix.sig(2) > eps
                        P = (Statistics(M,c,s) - ggmmix.mus(2)) ./ ggmmix.sig(2);
                    elseif ggmmix.sig(3) > eps
                        P = (Statistics(M,c,s) - ggmmix.mus(3)) ./ ggmmix.sig(3);
                    else
                        error('GGMmix failed')
                    end
                    
                    % Diagnostics
                    h = img2hist({Statistics(M,c,s), P}, [], 'Contrast distributions');
                    pause(0.1)
